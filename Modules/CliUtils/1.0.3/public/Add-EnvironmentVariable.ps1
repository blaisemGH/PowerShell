...
function Add-EnvironmentVariable {
    ...
    if ( $Name -match 'Path') {
        [string[]]$currentPaths = $currentEnvVarValue -split [Path]::PathSeparator
    ...
    if ( $confirm -ne 'n' ) {
        $setEnvParams.Value = Set-EnvironmentVariableValue -Name $Name -Value $setEnvParams.Value -CurrentPaths $currentPaths
...
function Set-EnvironmentVariableValue {
    ...
    if ( $Name -match 'Path' ) {
        return $Value + ($currentPaths | where {$_ -notin $newPaths}) -join [Path]::PathSeparator
    }
    else {
        return [string]$Value
    }
