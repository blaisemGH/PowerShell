...
class GCloudCompletion {
    ...
    [IEnumerable[CompletionResult]] GetCompletions(
        ...
        if ( ![GCloudCompletion]::CompletionTree -or ![GCloudCompletion]::CompletionTree.Keys.Count ) {
            [GCloudCompletion]::ImportCompletionTree()
        }
        
        $currentCliTreeLevel = [GCloudCompletion]::CompletionTree.Clone() # Will be changing $currentCliTreeLevel, so need a clone to not corrupt the cache.
    ...
    static [void] ImportCompletionTree() {
        $cachedCompletionFile = [GCloudCompletion]::GcloudVersion.ToString() + '_' + (Split-Path ([GCloudCompletion]::CompletionFilepath) -Leaf )
        $completionCacheFilepath = Join-Path ([GCloudCompletion]::CompletionsWorkDir) $cachedCompletionFile

        if ( !(Test-Path $completionCacheFilepath) ) {
            [GCloudCompletion]::NewCompletionCacheFile($completionCacheFilepath)
        }

        [GCloudCompletion]::CompletionTree = Get-Content $completionCacheFilepath -Raw | ConvertFrom-Json -AsHashtable
        
        $configPropertyCompletions = [GCloudCompletionPropertiesBuilder]::new( (gcloud topic configurations) )
        
        $propertyCompletions = $configPropertyCompletions.GetConfigPropertyCompletions()
        $pyCompletionTree = [GCloudCompletion]::CompletionTree.Clone()
        ...
        foreach ($configCommand in $pyCompletionTree.commands.config.commands.Keys ){
            foreach ( $kvpair in $propertyCompletions.GetEnumerator() ) {
                [GCloudCompletion]::CompletionTree.commands.config.commands.$configCommand.commands.Add( $kvPair.Key, $kvPair.Value)
            }
        }
    }
    ...
    static [void] NewCompletionCacheFile([string]$completionCacheFilepath) {
        ...
        $gcloudCompletionFile = Split-Path ([GCloudCompletion]::CompletionFilepath) -Leaf
        ...
        if ( $existingLocalCompletionFileVersions -notcontains [GCloudCompletion]::GcloudVersion.ToString() ) {

            '{' + (Get-Content ([GCloudCompletion]::CompletionFilepath) | Select-Object -skip 4) |
                ConvertFrom-Json |
...
class GCloudCompletionPropertiesBuilder {
    [string[]]$RelevantHelpMenuText
    
    GCloudCompletionPropertiesBuilder([string[]]$helpMenuToParse) {
