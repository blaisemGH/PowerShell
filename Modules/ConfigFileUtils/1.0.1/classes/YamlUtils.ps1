...
Class YamlUtils {
    ...
    [void] Split_CurrentLine ( [string]$line ) {
        ...
        if ($this.linecontent -match 'effect: NoSchedule') {
            write-host 'yes'
            write-host 'yes'
            write-host 'yes'
        }
    ...
    [object] Import_RawYamlCode () {
        ...
        try {
            ...
            write-host $exceptionLineNumber
            write-host ($this.debugMapInputLineCount | Out-String)
            write-host $parsedCode
            write-host $parsedCode.count
            ...
            write-host $minimumExceptionLineNumber
