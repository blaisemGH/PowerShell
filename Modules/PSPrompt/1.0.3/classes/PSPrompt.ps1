...
#>
class PSPrompt {
    static [regex]$RegexAnsiReset = [regex]::new("`e\[0m", 'Compiled')
    static [regex]$RegexAllAnsi = [regex]::new("`e[^m]+m", 'Compiled')
    ...
    static [string] GetPrompt(){
        ...
        $promptLines = foreach ( $line in $itemLines ) {
            ...
            ) -join '' -replace "($([PSPrompt]::RegexAnsiReset)\s*)*(?=$([PSPrompt]::RegexAnsiReset)$)"
            ...
            $lengthMiddleWhiteSpace = [Console]::WindowWidth - (
                ($leftPrompt -replace [PSPrompt]::RegexAllAnsi).length + ($rightPrompt -replace [PSPrompt]::RegexAllAnsi).length + $lineConnector.Length
            )
            ...
            $promptline = & {
                ...
                # Handle the case where leftprompt + rightprompt lengths exceed [Console]::WindowWidth. This forces a new line.
                    ...
                    $lineOpener = if ( [PSPromptConfig]::MultiLineConnectorOpenMiddleLine ) {
                        [PSPromptConfig]::MultiLineConnectorOpenMiddleLine + ' '
                    }
                    $consoleWidthMinusLineOpener = [Console]::WindowWidth - $lineOpener.Length
                    $indexCount = 1
                    $isFirstLine = $true
                    $leftPromptSplitOverMultilines = -join $(foreach ($char in $leftPrompt.ToCharArray() ) {
                        if ( $char -eq "`e" ) {
                            $ansiString = $true
                            $currentAnsiString = $char
                        }
                        if ( !$ansiString ) {
                            $indexCount++
                        } else {$currentAnsiString += $char}
                        if ($ansiString -and $char -eq 'm') {
                            $ansiString = $false
                            # Don't add m to $currentAnsiString. This will double print the m if the line ends exactly on the m.
                        }
                        
                        if ( $isFirstLine -and $indexCount % [Console]::WindowWidth -eq 0 ) {
                            $char + $ansiReset + [Environment]::NewLine + $lineOpener + $currentAnsiString
                            $isFirstLine = $false
                            $indexCount = 0
                        } elseif (!$isFirstLine -and $indexCount % $consoleWidthMinusLineOpener -eq 0 ) {
                            $char + $ansiReset + [Environment]::NewLine + $lineOpener + $currentAnsiString
                        } else {
                            $char
                        }
                    })
                    $skipWhiteSpace = ' ' * ($consoleWidthMinusLineOpener - ($rightPrompt -replace [PSPrompt]::RegexAllAnsi).Length)
                    (
                        $leftPromptSplitOverMultilines + $ansiReset +
                        [Environment]::NewLine + 
                        $lineOpener + 
                        $skipWhiteSpace + 
                        $rightPrompt
