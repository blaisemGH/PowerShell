...
class GcpPamEntitlementCompleter : IArgumentCompleter {
    ...
    [IEnumerable[CompletionResult]] CompleteArgument(
        ...
        if ( $argLocation -and $argTier ) {
            $cachedEntry = [GcpPamEntitlementCompleter]::cachedEntitlements.$cacheTier.$cacheId | where location -eq $location
            ...
            [GcpPamEntitlementCompleter]::cachedEntitlements.$cacheTier.$cacheId = $cachedMetaData
        }
...
class GcpPamEntitlementCompletionsAttribute : ArgumentCompleterAttribute, IArgumentCompleterFactory {

    [IArgumentCompleter] Create() {
        return [GcpPamEntitlementCompleter]::new()
    }
}