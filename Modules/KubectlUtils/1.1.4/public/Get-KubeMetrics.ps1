    ...
    begin {
        $filterNamespaces = @(
            if ( $Namespaces -contains '--all-namespaces' -or $Namespaces -contains '-A' ) {
                kubectl get namespaces -o name | foreach { $_ -replace '^namespace/'}
            } else {
                [HashSet[string]]$allNamespaces = kubectl get namespaces -o name | foreach { $_ -replace '^namespace/'}
                $allNamespaces.IntersectWith($Namespaces)
                $allNamespaces
            }
        )
