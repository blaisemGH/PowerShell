Function Replace-StringInFile {
    ...
    Get-ChildItem $Path @Params | ForEach {
        ...
        If ( !($fast) ) {
            ...
            $contents = (( Get-Content $file ) | ForEach {
                ...
                If ( $line -notmatch $noComments -and $line -match $oldPattern ) {
                    ...
                    Write-Host ( "`t`t" + 'new: ' ) -ForegroundColor 'cyan' -NoNewLine;    Write-Host ( $line + [Environment]::NewLine ) -ForegroundColor 'white'
                    $flag = $true
                }
                $line
            })
        ...
        Else {
            ...
            If ( $flag ) {
                Write-Host ("`t`t  " + 'String matches found! ') -ForegroundColor 'darkgreen' -NoNewLine; Write-Host ('File has been ') -NoNewLine; Write-Host 'updated' -ForeGroundColor 'Cyan' -NoNewLine; Write-Host ('.{0}' -f [Environment]::NewLine)
                $flag = $False
                Move-Item $tmpFile $file -Force
            }
            Else {
                Write-Host ('{0}No string match found. File will be ignored.{1}' -f "`t`t  ", [Environment]::NewLine )
                Remove-Item $tmpFile
            }
        }
    }
